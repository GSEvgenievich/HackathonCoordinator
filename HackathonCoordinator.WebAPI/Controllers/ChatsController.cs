    ...
    public class ChatsController : BaseApiController
    {
        ...
        public async Task<ActionResult<ApiResponse<ChatDto>>> GetTeamChat(int teamId)
        {
            ...
            var chat = await _context.Chats
                ...
                .Include(c => c.Type)
                .FirstOrDefaultAsync(c => c.Teams.Any(t => t.Id == teamId) && c.TypeId == 1);
        ...
        public async Task<ActionResult<ApiResponse<ChatDto>>> GetTaskChat(int taskId)
        {
            ...
            if (task == null)
                return HandleNotFound<ChatDto>("Задача не найдеwна");
            ...
            var hasAccess = user.RoleId == 3 ||
                          teamMembers.Any(u => u.Id == userId && u.RoleId == 1) ||
            ...
            var chat = await _context.Chats
                .Include(c => c.Messages)
                    ...
                    .ThenInclude(u => u.ProfileIcon)
                .Include(c => c.Type)
                .FirstOrDefaultAsync(c => c.Tasks.Any(t => t.Id == taskId) && c.TypeId == 2);
            ...
            var captain = teamMembers.FirstOrDefault(u => u.RoleId == 1);
            ...
            else if (chat.TypeId == 2) // Чат задачи
            {
                ...
                if (task != null)
                {
                    ...
                    hasAccess = user.RoleId == 3 ||
                               task.Team.Users.Any(u => u.Id == userId && u.RoleId == 1) ||
                               task.AssignedToId == userId;
                }
            }
        ...
        public async Task<ActionResult<ApiResponse>> DeleteMessage(int messageId)
        {
            ...
            if (message.UserId != userId && user?.RoleId != 3)
            ...
            int chatId, [FromQuery] int page = 1, [FromQuery] int pageSize = 50)
        {
            ...
            // Проверка доступа (аналогично другим методам)
            ...
            var messages = await _context.Messages
                ...
                .Take(pageSize)
                .OrderBy(m => m.SentAt) // Возвращаем в правильном порядке
        ...
        private async Task<bool> CheckChatAccess(Chat chat, int userId)
        {
            ...
            if (user.RoleId == 3) return true; // Организатор имеет доступ ко всем чатам
            ...
            else if (chat.TypeId == 2) // Чат задачи
            {
                ...
                if (task != null)
                {
                    return task.Team.Users.Any(u => u.Id == userId && u.RoleId == 1) ||
                           task.AssignedToId == userId;
                }
            }
